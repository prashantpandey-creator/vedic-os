...
pattern = re.compile(r"# Handle current state\s+if st\.session_state\.omni_state == \"GENERATING\":.*?st\.write\(f\"🦅 \*\*\[STEP", re.DOTALL)

# Use raw string so \n is kept as a backslash and an n
new_gen_block = r"""# Handle current state
        if st.session_state.omni_state == "GENERATING":
            ...
            if st.session_state.omni_step > 10:
                ...
                from core.checkpoint import build_phase_summary, save_checkpoint
...
if pattern.search(content):
    content = pattern.sub(new_gen_block, content)
    with open("/Users/badenath/projects/local-llm-ui/app.py", "w") as f:
        f.write(content)
    print("Deep iterations patched properly.")
else:
